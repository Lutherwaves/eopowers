...
2. В полето **"Ключова дума"** (textbox "Въведете дума/израз") въведи **"СМР"** и натисни бутон **"Добави"**.
...
**Забележка:** Филтърът "СМР" свива резултатите до ~200 строителни поръчки. По-нататъшното филтриране (положителни/отрицателни ключови думи) се прилага програмно при извличане на данни.
...
6. Изтегли всеки прикачен файл с натискане на бутона за сваляне (иконка 󰇚).
   **ВАЖНО:** Playwright сваля файлове в `.playwright-mcp/` с тирета вместо интервали.
   ...
   ```bash
   mv ".playwright-mcp/<downloaded-filename>" "./bloxpowers/offers/<offer-id>/attachments/"
   ```
...
10. **Timeout:** 60 секунди на поръчка. Ако не успее — запиши "—" за площ и продължи.
...
5. **Приоритет при множество категории:** Ако съвпада с повече от една, приложи: **ЕЕ > Топлоизолация > Ново строителство > Ремонт/реконструкция > останалите**. ЕЕ винаги печели ако присъстват ключови думи за енергийна ефективност (НПВУ, ЕСКБ, ЕСМ, енергийно обновяване, енергоспестяващи). "Обновяване" в контекста на ЕЕ НЕ е Ремонт.
